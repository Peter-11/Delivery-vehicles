...
)
{
	...
	pid->PID_param_init(pid,max_out,max_err,integral_limit,deadband,kp,ki,kd,A,B,improve);/* 输入pid各参数 */
}
/* pid修改函数 */
...
{
	pid->Output=0.9f*pid->Output+0.1f*pid->Last_Output;
}
/* 电机堵转控制实现函数 */
...
static void f_Error_Limit(PID_TypeDef *pid)
{
	if(pid->Err > pid->MaxErr) pid->Err -= 360 ;
	if(pid->Err < -pid->MaxErr) pid->Err += 360;
};
